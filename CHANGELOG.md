...
- **缓存升级**: 临时候选允许后续页面升级为更完整可信路径，最终证据禁止降级或被冲突路径覆盖；每类目最多3次升级扫描
- **跨页复用与隔离**: 最终证据直接复用，不同类目使用独立缓存键和提取锁
...
- 不增加商品详情页或其他 HTTP 请求
- S1专项新增30项；S1与V2.2.1类目测试共68项通过，相关回归100项通过，并发测试连续3次均为5项通过
- 完整测试连续两次均为304项通过、0失败（修复前基线274项）
- 真实 Boxe 验证受当前 Scrapling 会话环境错误阻断（HTTP 0，`No active session available`），未获得eMAG响应，未把它记为线上验证通过
