...
- 基于已验收的 V2.2.0 开发更完整的列表页真实类目层级提取。
- 当前状态为开发分支，尚未验收为稳定版本。

## V2.2.0（当前稳定，2026-08-11）

- products.json、products.csv、products.xlsx 使用统一简体中文产品字段名。
- `campaign_name` 输出为“链接打标”，`badges` 继续独立输出为“商品标签”。
- 扩展信息采用分层字段映射和无损键冲突保护，不覆盖或丢弃原值。
- 支持从商品已有真实路径拆分一级至五级类目，并保持其他商品数据不变。
- V2.2.0 最终完整测试为 236 项通过、0 失败。
